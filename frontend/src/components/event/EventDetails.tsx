...
export default function EventDetails({ event_id }: { event_id: string }) {
  ...
  const fetchEventDetails = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/events/${event_id}`); // ${process.env.NEXT_PUBLIC_API_URL}/events/${event_id} // http://minipc:3001/events/${event_id}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/events/${event_id}`, {
        method: 'PATCH',
